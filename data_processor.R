...
get.current.return <- function(
  ...
  performance.result              <- as.matrix( do.call( cbind, 
...
get.performance.metric <- function(
  ...
  Beta.SP500.Y                    <- cbind( SP500.beta.Y, treynor.sp500.Y, jensen.alpha.sp500.Y )

  Beta.SPY.Y                      <- cbind( SPY.beta.Y, treynor.spy.Y, Jensen.alpha.spy.Y )

  Beta.Y                          <- rbind( Beta.SPY.Y, Beta.SP500.Y )

  rownames( Beta.Y )              <- c( "SPY", "SP500" )

  colnames( Beta.Y )              <- c( "Market", "Treynor", "Jensen Alpha" )
  ...
  `CVaR Daily`                    <- c( mean( quantile( daily.return[ daily.return <= `VaR Daily`[1] ] ) ),

                                        mean( quantile( daily.return[ daily.return <= `VaR Daily`[2] ] ) )

                                      )

  `CVaR Yearly`                   <- `CVaR Daily` * sqrt( 252 )

  zscore                          <- qnorm( c( 0.05, 0.01 ) )
  
  parametric.VaR.D                <- daily.return.mu + ( zscore * daily.return.sigma )

  parametric.VaR.Y                <- annual.ret + ( zscore * annual.sigma )

  parametric.CVaR.D               <- daily.return.mu + ( zscore * daily.return.sigma )

  parametric.CVaR.Y               <- annual.ret + ( zscore * daily.return.sigma )
